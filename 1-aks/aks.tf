...
resource "azurerm_kubernetes_cluster" "ado" {
...
resource "azurerm_kubernetes_cluster_node_pool" "app_pool" {
  name                  = "apppool"
  kubernetes_cluster_id = azurerm_kubernetes_cluster.ado.id
  vm_size               = var.vm_size
  node_count            = var.node_count
  os_sku                = var.os_sku
  enable_auto_scaling   = var.enable_auto_scaling
  ...
  depends_on = [azurerm_kubernetes_cluster.ado]
}

resource "azurerm_role_assignment" "acr" {
  principal_id                     = azurerm_kubernetes_cluster.ado.kubelet_identity[0].object_id
